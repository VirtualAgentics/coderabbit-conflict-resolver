...
class ConflictResolver:
    ...
    def _detect_file_conflicts(self, file_path: str, changes: List[Change]) -> List[Conflict]:
        ...
        for i, change1 in enumerate(sorted_changes):
            ...
            if conflicting_changes:
                ...
                overlap_percentage = self._calculate_overlap_percentage(change1, conflicting_changes)
    ...
    def _classify_conflict_type(self, change1: Change, conflicting_changes: List[Change]) -> str:
        ...
        if len(conflicting_changes) == 1:
            ...
            if (change1.start_line == change2.start_line and 
                change1.end_line == change2.end_line):
                return "exact"
            elif (change1.start_line <= change2.start_line and 
                  change1.end_line >= change2.end_line):
                return "major"
            else:
                return "partial"
    ...
    def _calculate_overlap_percentage(self, change1: Change, conflicting_changes: List[Change]) -> float:
        ...
        total_size = max(change1.end_line, change2.end_line) - min(change1.start_line, change2.start_line) + 1
    ...
    def apply_resolutions(self, resolutions: List[Resolution]) -> ResolutionResult:
        ...
        success_rate = (applied_count / (applied_count + conflict_count)) * 100 if (applied_count + conflict_count) > 0 else 0
