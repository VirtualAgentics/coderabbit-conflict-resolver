...
This module tests the Claude CLI provider implementation including:
...
- Integration tests with real Claude CLI (optional, requires CLI installed)
"""
...
class TestClaudeCLIProviderProtocol:
    """Test that ClaudeCLIProvider conforms to LLMProvider protocol."""

    @patch("shutil.which")
    def test_provider_implements_protocol(self, mock_which: Mock) -> None:
        """Test that ClaudeCLIProvider implements LLMProvider protocol."""
...
class TestClaudeCLIProviderInitialization:
    """Test ClaudeCLIProvider initialization and configuration."""
    ...
    def test_init_with_valid_params(self, mock_which: Mock) -> None:
        ...
        provider = ClaudeCLIProvider(model="claude-opus", timeout=90)
    ...
    def test_init_error_includes_install_url(self, mock_which: Mock) -> None:
        ...
        with pytest.raises(LLMConfigurationError, match="https://claude.ai/cli"):
            ClaudeCLIProvider()
    ...
    def test_init_error_includes_details(self, mock_which: Mock) -> None:
        ...
        assert exc_info.value.details["provider"] == "claude-cli"
        assert exc_info.value.details["cli_command"] == "claude"


class TestClaudeCLIProviderTokenCounting:
...
class TestClaudeCLIProviderCostCalculation:
...
class TestClaudeCLIProviderUsageTracking:
...
class TestClaudeCLIProviderIntegration:
    """Integration tests with real Claude CLI (requires CLI installed)."""
